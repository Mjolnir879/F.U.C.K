...
class StatsBar(GUI):
    ...
    Esta versão é compatível com o modo online, recebendo os dados do jogador externamente.
    """
    def __init__(self, x, y, sizex, sizey, texture_empty, texture_full, stat_name, parent=None, orientation: str = 'horizontal', direction: str = 'positive'):
        ...
        # Carrega as texturas das barras
        ...
        self.orientation = (orientation or 'horizontal').strip().lower()
        self.direction = (direction or 'positive').strip().lower()

        # A barra agora guarda os valores de status internamente.
        # Eles são atualizados pela nova função update().
        self.current_value = 0
        self.max_value = 100

        print(f"StatsBar criada para status '{stat_name}' em ({x}, {y})")

    def update(self, player_data):
        """
        Atualiza os valores da barra com base nos dados do jogador recebidos.
        'player_data' deve ser o dicionário da entidade do jogador.
        """
        # Se não houver dados do jogador, a barra fica vazia por padrão.
        ...
        try:
            ...
            # Pega o valor atual do dicionário de stats
            self.current_value = stats.get(self.stat_name, 0)
            
            # Pega o valor máximo correspondente
            max_stat_name = f"max{self.stat_name.capitalize()}"
            self.max_value = stats.get(max_stat_name, 100)
            
            # Garante que o valor máximo seja pelo menos 1 para evitar divisão por zero
            if self.max_value <= 0:
                self.max_value = 1
                
        except Exception as e:
            print(f"Erro ao atualizar StatsBar para '{self.stat_name}': {e}")
            self.current_value = 0
            self.max_value = 100
    ...
    def draw(self):
        ...
        try:
            # Calcula a porcentagem com base nos valores já armazenados internamente
            fill_percentage = self.current_value / self.max_value if self.max_value > 0 else 0
    ...
    def _draw_fallback_bar(self, abs_x, abs_y, fill_percentage):
        ...
        glBegin(GL_QUADS)
        glVertex2f(abs_x, abs_y); glVertex2f(abs_x + self.sizex, abs_y)
        glVertex2f(abs_x + self.sizex, abs_y + self.sizey); glVertex2f(abs_x, abs_y + self.sizey)
        glEnd()
        
        # Preenchimento
        if fill_percentage > 0:
            if self.stat_name == "hp": glColor4f(0.0, 1.0, 0.0, 0.9)
            elif self.stat_name == "mana": glColor4f(0.0, 0.3, 1.0, 0.9)
            elif self.stat_name == "stamina": glColor4f(1.0, 1.0, 0.0, 0.9)
            else: glColor4f(1.0, 0.0, 1.0, 0.9) # Cor padrão rosa
            
            glBegin(GL_QUADS)
            if self.orientation == 'horizontal':
                fill_w = self.sizex * fill_percentage
                x0 = abs_x if self.direction == 'positive' else abs_x + (self.sizex - fill_w)
                glVertex2f(x0, abs_y); glVertex2f(x0 + fill_w, abs_y)
                glVertex2f(x0 + fill_w, abs_y + self.sizey); glVertex2f(x0, abs_y + self.sizey)
            else: # vertical
                fill_h = self.sizey * fill_percentage
                y0 = abs_y if self.direction == 'positive' else abs_y + (self.sizey - fill_h)
                glVertex2f(abs_x, y0); glVertex2f(abs_x + self.sizex, y0)
                glVertex2f(abs_x + self.sizex, y0 + fill_h); glVertex2f(abs_x, y0 + fill_h)
            glEnd()
    ...
    def _draw_textured_bar(self, abs_x, abs_y, fill_percentage):
        """Desenha barra usando texturas com mescla via Scissor Test"""
        if self.texture_empty:
            self.texture_empty.draw(abs_x, abs_y, 0, 1)
        ...
        try:
            ...
            if self.orientation == 'horizontal':
                ...
                sx = int(abs_x) if self.direction == 'positive' else int(abs_x + (self.sizex - fill_w))
                sy = int(screen_height - abs_y - self.sizey)
                sw, sh = fill_w, int(self.sizey)
            else: # vertical
                fill_h = max(1, int(self.sizey * fill_percentage))
                sx = int(abs_x)
                y_gui = abs_y if self.direction == 'positive' else abs_y + (self.sizey - fill_h)
                sy = int(screen_height - y_gui - fill_h)
                sw, sh = int(self.sizex), fill_h
...
class InterfaceManager:
    ...
    def update(self, mouse_x, mouse_y, mouse_pressed, player=None):
